...
$(document).ready(function(){
	...
	var schedule=function(league){
		...
		$('.week').each(function(i, elem){
			$(this).prepend("<h3>Week " + (i + 1) + "</h3>");
			$(elem).find(".span4").each(function(j, elem2){
				$(this).prepend("<h4>Game " + (j + 1) + "</h4>");
				...
				$(elem).find(".span4").each(function(j, elem2){
					...
					if(parseInt($('#' + i + league[team1].id).val()) > parseInt($('#' + i + league[team2].id).val())){
							...
							console.log($('#' + i + league[team1].id).val() + " is greater than " + $('#' + i + league[team2].id).val())
								$.ajax({
									url: '/backliftapp/teamdata/' + league[team1].id,
									type: 'PUT',
									dataType: 'JSON',
									data: {wins: function()
								...
								$.ajax({
									url: '/backliftapp/teamdata/' + league[team2].id,
									type: 'PUT',
									dataType: 'JSON',
...
// ********************************************* Deleting a Team ******************************************************
	...
	$('table').on("click", ".delete", function(event){
		$.ajax({
			url: 'backliftapp/teamdata/' + $(this).attr('id'),
			type: 'DELETE',
			success: function(league) {
